...
static void* fbitstr_open(char *file, char *mode)
{
    ...
    int fmode = FA_READ;
    if(*mode == 'w') fmode |= FA_WRITE;
    f_open(context->fp, file, fmode);
    if (!context->fp) {
        free(context);
        return NULL;
    }
...
static int fbitstr_seek(void *stream, long offset, int origin)
{
    ...
    context->bitbuf = 0;
    context->bitnum = 0;
    return f_lseek(context->fp, offset);
}
...
static int fbitstr_flush(void *stream)
{
    ...
    if (context->bitnum != 0) {
        if (EOF == f_putc(context->bitbuf & 0xff, context->fp)) {
            return EOF;
        }
        context->bitbuf = 0;
        context->bitnum = 0;
    }
    return f_sync(context->fp);
}
