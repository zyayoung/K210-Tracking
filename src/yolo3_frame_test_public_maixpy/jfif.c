...
int jfif_write(void *ctxt, FIL *fp)
{
    ...
    for (i=0; i<16; i++) {
        ...
        f_putc(i   , fp);
        for (j=0; j<64; j++) {
            f_putc(jfif->pqtab[i][ZIGZAG[j]], fp);
        }
    }
    ...
    f_putc(len >> 8, fp);
    f_putc(len >> 0, fp);
    f_putc(8   , fp); // precision 8bit
    f_putc(jfif->height >> 8, fp); // height
    f_putc(jfif->height >> 0, fp); // height
    f_putc(jfif->width  >> 8, fp); // width
    f_putc(jfif->width  >> 0, fp); // width
    f_putc(jfif->comp_num, fp);
    for (i=0; i<jfif->comp_num; i++) {
        f_putc(jfif->comp_info[i].id, fp);
        f_putc((jfif->comp_info[i].samp_factor_v << 0)|(jfif->comp_info[i].samp_factor_h << 4), fp);
        f_putc(jfif->comp_info[i].qtab_idx, fp);
    }
    ...
    for (i=0; i<16; i++) {
        ...
        f_putc(len >> 0, fp);
        f_putc(i + 0x10, fp);
        f_write(fp, jfif->phcac[i]->huftab, len - 3, NULL);
    }
    ...
    for (i=0; i<16; i++) {
        ...
        f_putc(len >> 0, fp);
        f_putc(i + 0x00, fp);
        f_write(fp, jfif->phcdc[i]->huftab, len - 3, NULL);
    }
    ...
    for (i=0; i<jfif->comp_num; i++) {
        f_putc(jfif->comp_info[i].id, fp);
        f_putc((jfif->comp_info[i].htab_idx_ac << 0)|(jfif->comp_info[i].htab_idx_dc << 4), fp);
    }
    f_putc(0x00, fp);
    f_putc(0x00, fp);
    f_putc(0x00, fp);
    ...
    // output data
    if (jfif->databuf) {
        f_write(fp, jfif->databuf, jfif->datalen, NULL);
    }
    ret = 0;
...
int jfif_save(void *ctxt, char *file)
{
    ...
    int   i, j;
    int   ret  = -1;
    f_open(fp, file, FA_CREATE_ALWAYS | FA_WRITE);
    if (!fp) goto done;
...
void* jfif_encode(BMP *pb)
{
    ...
    udst = yuv_datbuf[1];
    vdst = yuv_datbuf[2];
    int wh=jfif->width * jfif->height;
    for (i=0; i<pb->height; i++) {
        for (j=0; j<pb->width; j++) {
            rgb_to_yuv(bsrc[0], bsrc[wh], bsrc[wh*2], ydst, udst, vdst);
            bsrc += 1;
            ydst += 1;
            if (j & 1) {
                udst += 1;
